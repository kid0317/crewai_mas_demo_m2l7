...
- `app/crews/xhs_note/agents.py`：  
  - 通过 **get 工厂方法**（如 `get_xhs_visual_analyst()`、`get_xhs_image_editor()` 等）创建 Agent，**每次调用返回新实例**；LLM、tools、multimodal 等在本模块绑定，**角色文案（role/goal/backstory）来自 `app/crews/config/agents.yaml`**；
- `app/crews/xhs_note/tasks.py`：  
  - 定义与业务场景对应的 Task 及**按单图构建 Task 的工厂函数**（如 `build_visual_analysis_task`、`build_image_edit_task`）；内容阶段三任务通过 **get 工厂方法**（`get_task_content_strategy()`、`get_task_copywriting(content_strategy_task)`、`get_task_seo_optimization(...)`）每次创建新实例，Task 描述与预期输出从 `app/crews/config/tasks.yaml` 加载；
...
Agent 的 **角色文案（role、goal、backstory 等）** 配置在 `app/crews/config/agents.yaml`，**LLM、tools、multimodal** 等在 `app/crews/xhs_note/agents.py` 中绑定。**获取方式**：通过 **get 工厂方法** 按需创建，每次调用返回新的 Agent 实例（避免单例在并发下的状态共享），对应函数为：
...
- **工厂方法**：`get_task_content_strategy()` — 每次调用返回新 Task 实例。
- **YAML 配置名**：`task_content_strategy`
- **Agent**：通过 `get_xhs_growth_strategist()` 创建。
...
- **工厂方法**：`get_task_copywriting(content_strategy_task: Task)` — 依赖上游内容策略 Task，每次调用返回新实例。
- **YAML 配置名**：`task_copywriting`
- **Agent**：通过 `get_xhs_content_writer()` 创建。
- **context**：`[content_strategy_task]`（由调用方传入）。
- **输出**：`output_pydantic=XhsCopywritingOutput`

#### 5.5 搜索优化任务

- **工厂方法**：`get_task_seo_optimization(content_strategy_task: Task, copywriting_task: Task)` — 依赖上游两个 Task，每次调用返回新实例。
- **YAML 配置名**：`task_seo_optimization`
- **Agent**：通过 `get_xhs_seo_expert()` 创建。
- **context**：`[content_strategy_task, copywriting_task]`（由调用方传入）。
...
1. **阶段一 `_run_visual_analysis_phase`**  
   ...
   - 单 Crew：agents=`[get_xhs_visual_analyst()]`，tasks=上述列表，`Process.sequential`；  
...
2. **阶段二 `_run_image_edit_phase`**  
   ...
   - 单 Crew：agents=`[get_xhs_image_editor()]`，tasks=上述列表，`Process.sequential`；  
   - 解析得到每图 `XhsImageEditPlan` 与编辑方案总结字符串。

3. **阶段三 `_run_content_phase`**  
   - 先通过工厂方法创建本阶段 Task：`content_strategy_task = get_task_content_strategy()`，`copywriting_task = get_task_copywriting(content_strategy_task)`，`seo_task = get_task_seo_optimization(content_strategy_task, copywriting_task)`；  
   - 单 Crew：agents=`[get_xhs_growth_strategist(), get_xhs_content_writer(), get_xhs_seo_expert()]`，tasks=`[content_strategy_task, copywriting_task, seo_task]`，`Process.sequential`；  
...
- **开发步骤建议**：
  ...
  2. 在 `crews/xhs_note/agents.py` 中完成 5 个 Agent 的 **get 工厂方法**（如 `get_xhs_visual_analyst()`），复用 m2l5/m2l6 prompt 模版；
  3. 在 `crews/xhs_note/tasks.py` 中实现按图构建的工厂函数 + 内容阶段三任务的 **get 工厂方法**（如 `get_task_content_strategy()`）；
  4. 在 `crews/xhs_note/flows.py` 中实现三阶段编排（每阶段通过 get 方法创建 Agent/Task）及对外的 `run_xhs_note_flow`；
...
- **Agent**：角色文案在 `config/agents.yaml`，LLM/tools/multimodal 在 `agents.py`；**均通过 get 工厂方法**（如 `get_xhs_visual_analyst()`）创建，**每次调用返回新实例**，避免单例在并发下的状态共享。
- **Task**：按图构建的工厂函数 + 两个 summary Task + 内容阶段三 Task（`get_task_content_strategy()`、`get_task_copywriting(...)`、`get_task_seo_optimization(...)`），每次调用返回新实例；描述与预期输出来自 `config/tasks.yaml`。
- **流程**：三阶段、三个 Crew（视觉分析 → 编辑方案 → 内容策略/文案/SEO），每阶段创建 Crew 时通过上述 get 方法获取 Agent 与 Task；最终报告由 `_generate_final_report` 在 Python 内生成字符串。
